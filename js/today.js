const API_KEY = "pZs%2BQbjTvsjYdeBc40z2fD7QRRi3lynCg2GLLsEj2n8%2B1thhngNGdz5RitRWetZTfHRlD0Etu%2BGFU6rTWSmIHg%3D%3D";

function getDate(){                                     /* 10시 전 업데이트 x 어제의 날짜 가져오기*/
    let today = new Date();
...
function getYesterDate(){                       /* 10시 전 업데이트 x 어제의 날짜 가져오기*/
    let today = new Date();
...
function ParseData(date){
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState == 4) {
            ...
            for (let x = 0 ; x<4 ; x++){
                td = this.responseXML.getElementsByTagName(`${tag[x]}`)[0].childNodes[0].nodeValue;
                yd = this.responseXML.getElementsByTagName(`${tag[x]}`)[1].childNodes[0].nodeValue;

                doc = document.querySelector(`.information__box div:nth-child(${x+1}) span:nth-child(2)`);
                doc.innerHTML = td;
                doc = document.querySelector(`.information__box div:nth-child(${x+1}) span:nth-child(3)`);
                if((td-yd)>0){
                    doc.innerHTML = `↑ ${td-yd}`;
