...
function getDate() {
  ...
  let str = `${today.getFullYear()}년 ${
    today.getMonth() + 1
  }월 ${today.getDate()}일 ${hour}시 ${min}분 ${today.getSeconds()}초`;
  const text = document.querySelector(".clock-sub-header span:nth-child(3)");
  text.innerHTML = str;
}

function getUpdateDate() {
  let today = new Date();
  let doc = document.querySelector(".clock-sub-header span:first-child");
